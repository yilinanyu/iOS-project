...
{
     NSLog(@" before initializePageControl : %zd",self.currentPage);
    ...
    _pageControl.currentPage = before;
    
    if(!_indicatorDisabled) [self addSubview:_pageControl];
    
    
    NSLog(@" initializePageControl : %zd",_pageControl.currentPage);
    [self setCurrentPage:before];
